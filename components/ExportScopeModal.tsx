        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => setSelectedFormat('reconciliation')}
                disabled={isExporting}
                className={`flex-1 px-4 py-2.5 rounded-lg border-2 transition-all text-sm font-medium ${
                  selectedFormat === 'reconciliation'
                    ? 'border-indigo-600 bg-indigo-50 text-indigo-700'
                    : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50 text-gray-700'
                } disabled:opacity-50 disabled:cursor-not-allowed`}
              >
          ...
          <div className="text-xs text-gray-500 bg-gray-50 p-3 rounded-lg">
            {selectedFormat === 'csv' ? (
              <><strong>CSV Export includes:</strong> All original Excel columns (SN, Date, Description, Amount, GL Reference, Aging, Recon Type) + Sheet metadata (Department, Branch, Account, Currency) + Application metadata</>
            ) : (
              <><strong>Reconciliation Report includes:</strong> Formatted report with header metadata, grouped transactions by type (Debits/Credits, Internal/External), section totals, grand total, and signature fields</>  
            )}
          </div>
        </div>
